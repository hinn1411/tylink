...
- **Private URL visibility** (F8): every base-table item — public or private — keeps the exact same shape (`PK=URL#<shortCode>`, `SK=METADATA`), just with two extra attributes: `ownerId` and `visibility` (`PUBLIC` | `PRIVATE`). One `redirect/decode` Lambda handles both:
